...
The variable `Private` contains information on whether or not the institution (represented as a row in the dataset) is a private or not. The possible values are either "yes" or "no". Ideally, we would recieve this information through a `data dictionary` that was associated with the `College` dataset, but we may not necessarily know this. However, with this information, we can treat this variable as a `factor`.
...
-   **Create a new Quarto document entitled "Project_s1_[unityID(s)]"**

1)  Find a dataset

- Dataset Criteria
    - At least 200 observations and 5 variables
    - at least one variable of quantitative type and at least one variable of categorical type (NOTE: you can create new variables, e.g., categorizing scores into "high," "medium," and "low").
    - 
...
[Base R Cheatsheet](https://iqss.github.io/dss-workshops/R/Rintro/base-r-cheat-sheet.pdf) [Data Wrangling Cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf)
